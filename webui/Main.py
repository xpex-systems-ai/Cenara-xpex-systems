...
def # Cenara production mode: single-page studio, no route/auth prompt.
inject_official_theme()
...
def _cenara_provider_enabled(provider: str) -> bool:
    ...
    if provider == "frontier_ai":
        backend = (os.getenv("CENARA_FRONTIER_PROVIDER", "huggingface") or "huggingface").strip().lower()
...
def cenara_trigger_real_generation(task_id, payload, status_box=None):
    ...
    try:
        ...
        # Direct prompt-to-video path: skip legacy TTS/material pipeline when the
        # configured frontier backend can return a finished MP4.
        if payload.video_source == "frontier_ai" and (os.getenv("CENARA_FRONTIER_PROVIDER", "huggingface") or "huggingface").strip().lower() == "huggingface" and os.getenv("HF_TOKEN"):
            provider_attempts.append({"provider": "huggingface", "status": "started"})
            direct_output = _task_dir_for(task_id) / "cenara-frontier-master.mp4"
            try:
                prompt_text = (getattr(payload, "video_script", "") or getattr(payload, "video_subject", "") or "").strip()
                generate_huggingface_video_file(
                    prompt_text,
                    str(direct_output),
                    model=os.getenv("CENARA_HF_VIDEO_MODEL", "Wan-AI/Wan2.1-T2V-1.3B"),
                )
                if direct_output.is_file() and direct_output.stat().st_size > 100_000:
                    validation = ffprobe_validate_mp4(direct_output)
                    if validation.get("valid"):
                        provider_attempts[-1]["status"] = "success"
                        result.update(success=True, output_path=str(direct_output), logs=["Hugging Face MP4 master generated"], fallback_video_used=False)
                        cenara_write_task_status(task_id, state="completed", output_path=str(direct_output), media_ready=True, audio_ready=True, render_started=True, mp4_created=True, preview_ready=False, download_ready=False, mp4_created_preview_failed=False, fallback_video_used=False, provider_attempts=provider_attempts, safe_message="MP4 generativo criado diretamente pelo backend open-model.")
                        return result
                provider_attempts[-1]["status"] = "invalid_mp4"
            except FrontierMediaError as exc:
                provider_attempts[-1]["status"] = "failed"
                provider_attempts[-1]["detail"] = " ".join(str(exc).split())[:240]
                logger.warning(f"Cenara direct Hugging Face failed safely: {type(exc).__name__}")
            except Exception as exc:
                provider_attempts[-1]["status"] = "failed"
                provider_attempts[-1]["detail"] = type(exc).__name__
                logger.warning(f"Cenara direct frontier failed safely: {type(exc).__name__}")
...
def cenara_render_command_center(params, selected_tts_server):
    st.markdown(
        ...
        <section class="cenara-hero">
          <div class="cenara-eyebrow">CENARA · AI VIDEO STUDIO</div>
          <h1>Digite a ideia. A Cenara cria o vídeo.</h1>
          <div class="cenara-subtitle">Uma única tela. Um único prompt. Direção por IA, geração de vídeo, preview e MP4.</div>
        </section>
        """,
        unsafe_allow_html=True,
    )
    ...
    status_cols[0].metric("Diretor IA", "Pronto" if os.getenv("OPENROUTER_API_KEY") else "Fallback")
    status_cols[1].metric("Vídeo IA", "Pronto" if os.getenv("HF_TOKEN") else "Fallback local")
    status_cols[2].metric("FFmpeg", "Pronto" if cenara_resolve_ffmpeg_binary() else "Ausente")
    status_cols[3].metric("Armazenamento", "Persistente")

    left, right = st.columns([1.25, 1], gap="large")
    with left:
        st.subheader("Criar vídeo")
        prompt = st.text_area(
            "Prompt do vídeo",
            key="cenara_single_prompt",
            height=220,
            placeholder="Ex.: Crie um vídeo cinematográfico apresentando a XPeX Academy como uma escola de IA futurista, com ambiente premium, iluminação azul e laranja, câmera suave e tecnologia de última geração.",
        )
        c1, c2, c3 = st.columns(3)
        with c1:
            estilo = st.selectbox("Estilo", ["Cinemático", "Institucional", "Comercial", "Educativo", "Redes Sociais"], key="cenara_single_style")
        with c2:
            formato = st.selectbox("Formato", ["16:9", "9:16", "1:1"], key="cenara_single_aspect")
        with c3:
            duracao = st.selectbox("Duração", [5, 8, 10, 15], index=2, key="cenara_single_duration", format_func=lambda value: f"{value}s")

        submitted = st.button("✨ Gerar vídeo", type="primary", use_container_width=True, disabled=not prompt.strip())

        if submitted:
            task_id = str(uuid4())
            payload = VideoParams(video_subject=prompt.strip())
            payload.video_script = prompt.strip()
            payload.video_terms = prompt.strip()
            payload.video_source = "frontier_ai" if os.getenv("HF_TOKEN") else "local"
            payload.video_clip_duration = int(duracao)
            payload.video_aspect = {
                "16:9": VideoAspect.landscape,
                "9:16": VideoAspect.portrait,
                "1:1": VideoAspect.square if hasattr(VideoAspect, "square") else VideoAspect.portrait,
            }.get(formato, VideoAspect.landscape)
            payload.voice_name = ""
            payload.subtitle_enabled = False
            payload.bgm_type = "none"
            payload.video_count = 1

            with st.status("Cenara está criando o vídeo...", expanded=True) as status_box:
                status_box.write(f"Estilo: {estilo}")
                status_box.write("Direção e renderização iniciadas")
                ...
                if result.get("success"):
                    ...
                    st.error(result.get("error") or "Não foi possível concluir a geração.")
...
# Legacy technical configuration UI intentionally hidden in production.
