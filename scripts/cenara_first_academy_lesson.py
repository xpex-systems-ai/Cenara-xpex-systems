...
MARKER = ROOT / "academy-first-lesson-v2.done"
...
def main() -> int:
    if os.getenv("CENARA_ACADEMY_FIRST_LESSON_ON_START", "0") != "1":
        ...
        return 0
    if MARKER.exists():
        print(f"CENARA_ACADEMY_FIRST_LESSON_V2 PASS existing=true marker={MARKER}")
        return 0

    print("CENARA_ACADEMY_FIRST_LESSON_V2 stage=start")
    try:
        ...
        print(f"CENARA_ACADEMY_FIRST_LESSON_V2 FAILED detail={str(exc)[:320]}")
        return 2
    except Exception as exc:
        print(f"CENARA_ACADEMY_FIRST_LESSON_V2 FAILED unexpected={type(exc).__name__} detail={str(exc)[:260]}")
        return 3

    MARKER.parent.mkdir(parents=True, exist_ok=True)
    MARKER.write_text(str(output), encoding="utf-8")
    print(
        "CENARA_ACADEMY_FIRST_LESSON_V2 PASS "
